...
«Ученики» → пакет → **«Сдвинуть номера»**: вручную сдвинуть номера вперёд или назад на любое число,
начиная с выбранного занятия (оно и все следующие; 8/8 +1 → 1/8 следующего пакета). Даты не меняются.
Текущим пакетом считается последний уже начавшийся (следующий пакет, заранее поставленный в
расписание, не подменяет идущий).
