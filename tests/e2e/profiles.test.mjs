...
test("профиль и разовые ссылки не попадают в кабинеты родителей/учеников", async () => {
  ...
  for (const secret of ["SECRET", "SECRET2", "SECRET3", "telemost", "miro"]) assert.equal(v.includes(secret), false, secret);
  await app.close();
});
